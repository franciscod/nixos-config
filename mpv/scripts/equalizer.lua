...
--[[
mpv 6-bands equalizer with visual feedback.
...
--]]
-- ------ config -------
local start_keys_enabled = true  -- if true then choose the up/down keys wisely
local key_leader = '\\-e'
local key_help  = key_leader..'-h'
local key_toggle_bindings  = key_leader..'-k'  -- enable/disable equalizer key bindings
local key_toggle_equalizer = key_leader..'-e'  -- enable/disable equalizer
local key_reset_equalizer  = key_leader..'-r'  -- sets all bands to gain 0
...
local bands = {
  ...
  --      {up  down}
  {keys = {'1', '!'}, filter = {'equalizer=f=64:width_type=o:w=3.3:g=', 0}}, -- 20-200
  {keys = {'2', '@'}, filter = {'equalizer=f=400:width_type=o:w=2.0:g=', 0}}, -- 200-800
  {keys = {'3', shr}, filter = {'equalizer=f=1250:width_type=o:w=1.3:g=', 0}}, -- 800-2k
  {keys = {'4', '$'}, filter = {'equalizer=f=2830:width_type=o:w=1.0:g=', 0}}, -- 2k-4k
  {keys = {'5', '%'}, filter = {'equalizer=f=5600:width_type=o:w=1.0:g=', 0}}, -- 4k-8k
  {keys = {'6', '^'}, filter = {'equalizer=f=12500:width_type=o:w=1.3:g=', 0}} -- - 20k
}

local
function braille(n)
  local h = math.floor(n / 64)
  local l = n - h * 64
  return string.char(0xe2)..string.char(0x80 + 0x20 + h)..string.char(0x80 + l)
end

local function brailleh(r, n)
  return (({
    {0, 1, 8, 9},
    {0, 2, 16, 18},
    {0, 4, 32, 36},
...
local function woot(p, v)
    local bm = 0

    if p < 0 then
      if v <= p then return 3 end  -- **
      if v == p+1 then return 2 end -- _*
    end
    ...
    if p > 0 then
      if p < v then return 3 end  -- **
      if v == p then return 1 end -- *_
    end
...
local function eqvizrow(vs, rl, rh, rd)
  local bs = ""
  for n=1,40 do
    local p = n*2-40
    local bc = 0
...
local function eqviz()
  local vs = {}
  for i=1,6 do
    vs[i] = filter_data(bands[i].filter)[2]
  end
  vs[7] = nil
  vs[8] = 0
  local bs = ""
  bs = bs.."[eq] "..eqvizrow(vs, 1, 4, 0).."\n"
  bs = bs.."[eq] "..eqvizrow(vs, 5, 8, -4)
  return bs
end
...
local function updateOSD()
  ...
  for i = 1, #bands do
    ...
    msg_cli = msg_cli .. "  " .. data[1] .. "hz: " .. data[2]
  end
...
local function toggle_bindings(explicit, no_osd)
  ...
  if not explicit then
    mp.osd_message("[eq] " .. (bindings_enabled and "bindings enabled" or "bindings disabled"))
  end
  for i = 1, #bands do
...
local function toggle_equalizer()
  ...
  mp.osd_message("[eq] " .. (eq_enabled and "eq enabled" or "eq disabled"))
  updateAF()
  -- updateOSD()
end
...
local function help()
  mp.osd_message(""
    .. " increase/decrease: 1-5/+shift"
    .. " toggle bindings:   " .. key_toggle_bindings
    .. " toggle equalizer:  " .. key_toggle_equalizer
    .. " reset equalizer:   " .. key_reset_equalizer
  )
end
print("help: " .. key_help)
mp.add_forced_key_binding(key_help, "help", help)
mp.add_forced_key_binding(key_toggle_equalizer, "toggle_equalizer", toggle_equalizer)
mp.add_forced_key_binding(key_toggle_bindings, "toggle_bindings", toggle_bindings)
mp.add_forced_key_binding(key_reset_equalizer, "reset_equalizer", reset_equalizer)
